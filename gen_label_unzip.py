...
scans_dir = "/cluster/scratch/zhoum/scannet/scans/"
for scene_id in os.listdir(scans_dir):
	if not (scene_id.endswith(".zip") and "scene" in scene_id):
		continue
	with ZipFile(scans_dir+scene_id, 'r') as zipObj:
		zipObj.extractall(scans_dir)
	scene_id = scene_id[:-4]
	scene_dir = scans_dir+scene_id+"/"
	label_dir = scans_dir+"label"+scene_id[5:]+"/"
	with ZipFile(scene_dir + scene_id+'_2d-instance.zip', 'r') as zipObj:
		zipObj.extractall(scene_dir)
	print ("unzipped")
	...
	for objectId in obj_list:
		...
		if not os.path.exists(label_dir):
			os.mkdir(label_dir)
		obj_mesh = label_dir+'scannet_mesh'+str(objectId)+'.ply'
		...
		for pose_txt in os.listdir(pose_dir):
			...
			if not os.path.exists(label_dir+"labels"+str(objectId)+"_" + label):
				os.mkdir(label_dir+"labels"+str(objectId)+"_" + label)
			fw = open(label_dir+"labels"+str(objectId)+"_" + label + "/"+pose_txt,"w+")
		...
		# print (occlude_count)
		# print ("****")
	shutil.make_archive(label_dir, 'zip', label_dir)
